...
const MyAppointments = () => {
    ...
    const [status, purchase_order_id] = useState([]);
    ...
    const initPay = (order) => {
        const options = {
            ...
            handler: async (response) => {
                ...
                try {
                    const { data } = await axios.post(
                        backendUrl + "/api/user/verifyKhalti",
                        response,
                        { headers: { token } }
                    );
                    if (data.success) {
    ...
    // Function to make payment using khalti-backend
    const appointmentKhalti = async (appointmentId) => {
        try {
            ...
            if (data.success) {
                window.location.href = data.paymentDetails.payment_url;
                //initPay(data.paymentDetails);
            } else {
                toast.error(data.message);
            }
        } catch (error) {
    ...
    //             website_url: "https://test-pay.khalti.com/",
    //             amount: 500,
    ...
    //             merchant_extra: "Additional data",
    //         };
